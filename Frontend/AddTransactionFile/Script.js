...
transactionForm.addEventListener('submit', async function (event) {
    ...
    // Create transaction object (matching backend schema)
    const transactionData = {
        type: type,
        amount: amountValue,
        category: type === 'Income' ? 'Salary' : 'Food', // Default category
    ...
    try {
        // ✅ CORRECT: Use fetch() with the URL
        ...
        showMessage(error.message || 'Failed to add transaction. Please try again.', 'error');
    } finally {
        // Reset button state
